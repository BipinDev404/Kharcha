...
bad = """                var showDeveloperDialog by remember { mutableStateOf(false) }
                SettingsRow(
...
good = """                HorizontalDivider(modifier = Modifier.padding(vertical = 16.dp), color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.1f))
                        ...
                        androidx.compose.ui.window.Dialog(
                            ...
                            Box(modifier = Modifier.fillMaxSize().background(Color.Black.copy(alpha = 0.9f)).clickable { isFullScreenImage = false }) {
                                ...
                                IconButton(
                                    ...
                                    Icon(Icons.Filled.Check, contentDescription = "Close", tint = Color.White) // wait we need a close icon but I'll just use what's there
                                }
                            }
                        }
                    }
...
if bad in text:
    print("Found Developer section, replacing!")
